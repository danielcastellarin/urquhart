...
MatrixXd callHomography(vector<vector<pair<double, double>>> correspondences)
{
    ...
    H = MatrixXd::Map(svd.matrixV().transpose().row(8).data(), 3, 3).transpose();
...
MatrixXd newHomography(std::vector<std::pair<std::vector<double>, std::vector<double>>> correspondences)
{
    assert(correspondences.size() >= 4);

    // TODO parallelize?
    double meanX1 = 0, meanY1 = 0, meanX2 = 0, meanY2 = 0;
    for (const auto& pointPair : correspondences)
    {
        meanX1 += pointPair.first[0];
        meanY1 += pointPair.first[1];
        meanX2 += pointPair.second[0];
        meanY2 += pointPair.second[1];
    }
    ...
    // TODO Can I assume the scalar is one?
    ...
    for (int i = 0; i < correspondences.size(); i++)
    {
        Vector3d norm1(correspondences[i].first[0], correspondences[i].first[1], 1);
        Vector3d norm2(correspondences[i].second[0], correspondences[i].second[1], 1);
...
int main()
{
	...
	MatrixXd ph = callHomography(ranCorr);
	cout << "Homography:\n" << ph << endl;
